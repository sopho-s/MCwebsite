...
<html>
    <head>
        ...
        <title>
            Monthly Challenge (Challengenum) Source Code
        </title>
    ...
    <body>
        ...
        <div class="versions">
            ...
            <table>
                ...
                <tr>
                <?php
                $dir = "versions/(Challengenum)";
                $files = scandir($dir, SCANDIR_SORT_NONE);
                usort($files, function($a,$b){return filesize( "versions/(Challengenum)/" . $a) <=> filesize( "versions/(Challengenum)/" . $b);});
                foreach ($files as $file) {
                    if (strlen($file) > 2) {
                        $uploadtime = date("d F Y H:i:s.", filemtime("versions/(Challengenum)/$file"));
                        $uploadsize = filesize("versions/1/$file") / 1000;
                        $uploadsize = (int) $uploadsize;
                        echo "<tr><td><a href=\"versions/(Challengenum)/$file\" download>$file</a></td><td>$uploadtime</td><td>$uploadsize KB</td></tr>";
                    }
                }
                ?>
                </tr>
